...
export default function Home() {
  ...
  const [packsRef, packsVisible] = useReveal()
  const [framesRef, framesVisible] = useReveal()
    ...
    <div>
      ...
      <section className="relative overflow-hidden">
        ...
        <div className="relative mx-auto max-w-6xl px-4 pb-20 pt-24 text-center sm:pt-32">
          <div
            className="hero-item mb-6 inline-flex items-center gap-2.5 rounded-full border border-primary-500/30 bg-primary-500/10 px-4 py-1.5 text-sm text-primary-300"
          ...
          <h1
            className="hero-item mx-auto max-w-3xl text-4xl font-extrabold leading-tight tracking-tight sm:text-5xl lg:text-6xl"
          ...
          <p
            className="hero-item mx-auto mt-6 max-w-xl text-lg text-surface-400"
      ...
      {/* Categories */}
      <section
        ref={catRef as React.RefObject<HTMLElement>}
      ...
      <section
        ...
        className="border-y border-surface-800 bg-surface-900/30"
      >
        <div className="mx-auto max-w-6xl px-4 py-16">
          <h2 className={`mb-10 text-center text-2xl font-bold sm:text-3xl reveal ${featVisible ? 'is-visible' : ''}`}>
            Simple de verdad
          </h2>
          <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
            {FEATURES.map((f, i) => (
              <div
                key={f.title}
                className={`rounded-xl border border-surface-800 bg-surface-900/50 p-5 reveal ${featVisible ? 'is-visible' : ''}`}
      ...
      {/* Featured packs */}
      <section
        ref={packsRef as React.RefObject<HTMLElement>}
        className="mx-auto max-w-6xl px-4 py-16"
      >
        <div className={`mb-10 flex items-end justify-between reveal ${packsVisible ? 'is-visible' : ''}`}>
          <div>
        ...
        <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-4">
          {featuredPacks.map((pack, i) => (
            <div
              key={pack.id}
              className={`reveal ${packsVisible ? 'is-visible' : ''}`}
      ...
      {featuredFrames.length > 0 && (
        <section
          ref={framesRef as React.RefObject<HTMLElement>}
          className="mx-auto max-w-6xl px-4 pb-16"
        >
          <div className={`mb-10 flex items-end justify-between reveal ${framesVisible ? 'is-visible' : ''}`}>
            <div>
          ...
          <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-4">
            {featuredFrames.map((frame, i) => (
              <div
                key={frame.id}
                className={`reveal ${framesVisible ? 'is-visible' : ''}`}
                style={{ transitionDelay: `${i * 90}ms` }}
              >
                <PackCard pack={frame} />
              </div>
      ...
      >
        <div className={`rounded-2xl border border-primary-500/20 bg-gradient-to-br from-primary-900/30 to-surface-900 p-8 text-center sm:p-12 reveal ${ctaVisible ? 'is-visible' : ''}`}>
